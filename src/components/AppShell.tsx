...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
          ...
          <nav className="grid gap-1 border-t border-border/60 px-4 py-3 md:hidden">
            {NAV.map((item) => (
              <Link
                ...
                className="rounded-lg px-3 py-2.5 text-sm text-muted-foreground hover:bg-accent hover:text-foreground [&.active]:bg-accent [&.active]:text-foreground"
              >
                {item.label}
              </Link>
            ))}
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-5 sm:py-10">{children}</main>
