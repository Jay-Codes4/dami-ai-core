        ...
        <section className="grid items-center gap-8 rounded-3xl border border-border/60 bg-card p-6 shadow-[var(--shadow-soft)] md:grid-cols-[1fr_260px] md:p-10">
          <div>
            ...
            <p className="mt-4 max-w-xl text-sm leading-relaxed text-muted-foreground">
              Dami can float on your desktop, wake when you say “Hey Dami”, listen to your question and bring the legal research back without making you open the browser first.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Button disabled size="lg">
                <Download className="mr-2 h-5 w-5" /> Windows installer coming next
              </Button>
              <Button asChild variant="outline" size="lg"><Link to="/ask">Use Dami on the web</Link></Button>
            </div>
            <p className="mt-3 text-xs text-muted-foreground">
              The desktop experience is being packaged as an installer. The web app remains fully usable while that build is prepared.
            </p>
          </div>
