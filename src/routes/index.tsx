  ...
  head: () => ({
    meta: [
      { title: "Dami AI — A Voice for Justice across Africa" },
      {
        name: "description",
        content:
          "Voice-first, code-switching African legal AI powered by Sahara speech and source-grounded research.",
      },
    ],
  }),
  component: Home,
...
function useMobile() {
  ...
  useEffect(() => {
    ...
    const update = () =>
      setMobile(media.matches || /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent));
    update();
...
function Home() {
  ...
  return (
    <AppShell>
      <section className="flex flex-col items-center gap-5 py-3 text-center sm:gap-8 sm:py-6">
        <DamiAvatar state="welcome" size={mobile ? 180 : 240} />
        ...
        <section className="mt-12 rounded-3xl border border-primary/20 bg-primary/5 p-6 md:flex md:items-center md:justify-between md:gap-8 md:p-8">
          <div className="max-w-2xl">
            <p className="text-sm font-semibold text-primary">
              Want an interactive Dami on your PC?
            </p>
            <h2 className="mt-2 text-2xl font-semibold">Install Dami Desktop</h2>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              Keep Dami floating on your desktop. Say “Hey Dami”, or click/double-click her for the
              same reliable voice workflow.
            </p>
          </div>
          <div className="mt-5 flex shrink-0 gap-2 md:mt-0">
            ...
            <Button asChild size="lg" variant="outline">
              <Link to="/desktop">Learn more</Link>
            </Button>
          </div>
        </section>
      )}
      <section className="mt-9 grid gap-3 sm:mt-14 md:grid-cols-3">
        {[
          {
            icon: Mic,
          ...
          },
          {
            ...
            body: "Dami prioritizes useful legal sources and shows the authorities behind the answer.",
          },
          {
            icon: ShieldCheck,
            title: "Useful, not overconfident",
            body: "Dami identifies jurisdiction limits and separates verified law from uncertainty.",
          },
      ...
      {sessions.length > 0 && (
        <section className="mt-10 space-y-4 sm:mt-14">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold">Recent research</h2>
            <Link to="/research" className="text-sm text-primary">
              View all
            </Link>
          </div>
          <div className="grid gap-3 md:grid-cols-3">
            {sessions.map((session) => (
              <Card key={session.id}>
                <CardHeader>
                  <CardTitle className="text-sm">{session.question}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  <p className="line-clamp-3">{session.answer.answer}</p>
                  <Link to="/research" className="inline-flex items-center gap-1 text-primary">
