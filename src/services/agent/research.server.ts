...
function renderExaEvidence(results: ExaResult[]) {
  return results
    .map((r, i) => {
      const url = r.url ?? "",
        highlights = (r.highlights ?? []).join("\n").trim(),
        evidence = (highlights || r.text || "No extracted passage available.").slice(0, 1200);
      return [
        `[S${i + 1}] ${r.title || hostFromUrl(url)}`,
        `URL: ${url}`,
...
async function callGroq(question: string, groqKey: string, evidence?: string): Promise<string> {
      ...
      messages: [
        {
          role: "system",
          content: grounded
            ? `${IDENTITY_PROMPT}\n\nReason from the supplied LIVE SOURCES and cite relied-on evidence inline as [S1], [S2], etc. Use only the strongest relevant sources. Never invent sources. If evidence is insufficient, state what is missing naturally and briefly.`
            : `${IDENTITY_PROMPT}\n\nLive web evidence is unavailable. Do not claim you searched the internet or fabricate citations. Give careful legal assistance from model knowledge and naturally flag only material facts needing current-source verification.`,
        },
        {
          role: "user",
  ...
  if (!response.ok) {
    ...
    if (response.status === 404)
      throw new ResearchError("Dami's configured Groq model is unavailable for this account.", 503);
    if (response.status === 429)
      throw new ResearchError(
        "Dami's free reasoning quota is temporarily busy. Try again shortly.",
        429,
      );
  ...
  answer = answer
    ...
    .trim();
  if (!answer)
    throw new ResearchError("Dami couldn't put that answer together. Please try again.", 502);
  return answer;
}
async function researchWithExaAndGroq(
  ...
  const citations = exaCitations(results),
    answer = await callGroq(question, groqKey, renderExaEvidence(results));
  return {
    answer,
    keyFindings: [],
    citations,
    insufficientEvidence: !citations.length,
    limitations:
      "Live legal research can change. Verify critical authorities at the linked primary source before professional reliance.",
  };
}
